...
-(id) init
{
	if ((self = [super init]))
	{
		...
		player = [CCSprite spriteWithFile:@"bison.png"];
		...
		scoreLabel = [CCLabelTTF labelWithString:startVal fontName:@"HelveticaNeue-Light" fontSize:50];
        ...
        // Run the displayAchievement method
        [self displayAchievement];
        NSLog(@"displayAchievement method called!");
		
		// Play the background music in an endless loop.
	//	[[SimpleAudioEngine sharedEngine] playBackgroundMusic:@"blues.mp3" loop:YES];
		
		// (Commented out] Preload the sound effect into memory so there's no delay when playing it the first time.
...
-(void) initSpiders
{
	...
	CCSprite* tempSpider = [CCSprite spriteWithFile:@"rock.png"];
	...
	for (int i = 0; i < numSpiders; i++)
	{
		...
		CCSprite* spider = [CCSprite spriteWithFile:@"pine.png"];
        CCSprite* rock = [CCSprite spriteWithFile:@"rock.png"];
        CCSprite* grass = [CCSprite spriteWithFile:@"grass.png"];
		[self addChild:spider z:0 tag:0];
        [self addChild:rock z:0 tag:1];
        [self addChild:grass z:0 tag:2];
		
		// Also add the spider to the spiders array so it can be accessed more easily.
		[spiders addObject:spider];
        [spiders addObject:rock];
        [spiders addObject:grass];
	}
...
-(void) resetSpiders
{
	...
	for (NSUInteger i = 0; i < numSpiders; i++)
	{
        ...
        CCSprite* rock = [spiders objectAtIndex: i];
        CCSprite* grass = [spiders objectAtIndex:i];
		spider.position = CGPointMake(imageSize.width * i + imageSize.width * 0.5f, screenSize.height + imageSize.height);
		spider.scale = 1;
		rock.position = CGPointMake(imageSize.width * i + imageSize.width * 0.5f, screenSize.height + imageSize.height);
        rock.scale = 1;
        grass.position = CGPointMake(imageSize.width * i + imageSize.width * 0.5f, screenSize.height + imageSize.height);
        grass.scale = 1;
		// It might still be moving, it'll have to stop.
		[spider stopAllActions];
        [rock stopAllActions];
        [grass stopAllActions];
	}
...
-(void) spidersUpdate:(ccTime)delta
{
	...
	for (NSUInteger i = 0; i < 10; i++)
	{
        ...
        int randomRockIndex = CCRANDOM_0_1() * [spiders count];
        int randomGrassIndex = CCRANDOM_0_1() * [spiders count];
		CCSprite* spider = [spiders objectAtIndex:randomSpiderIndex];
		CCSprite* rock = [spiders objectAtIndex:randomRockIndex];
        CCSprite* grass = [spiders objectAtIndex:randomGrassIndex];
		...
		if ([spider numberOfRunningActions] == 0)
		{
            ...
            {
                CCLOG(@"Dropping a rock after %i retries.", i);
            }
            ...
            if ([grass numberOfRunningActions] == 0)
            {
                CCLOG(@"Dropping a grass after %i retries.", i);
            }
...
-(void) runSpiderMoveSequence:(CCSprite*)spider
{
	...
	CCSequence* sequence = [CCSequence actions:easeEnd, callDidDrop, nil];
...
-(void) runRockMoveSequence:(CCSprite*)rock
{
	...
	// This is the sequence which controls the spiders' movement. A CCCallFuncN is used to reset the
	// spider once it has moved outside the lower border of the screen, which is when it can be re-used.
	CGPoint belowScreenPosition = CGPointMake(rock.position.x, -(3 * [rock texture].contentSize.height));
	CCMoveTo* moveEnd = [CCMoveTo actionWithDuration:spiderMoveDuration position:belowScreenPosition];
	CCEaseBackInOut* easeEnd = [CCEaseBackInOut actionWithAction:moveEnd];
	CCCallFuncN* callDidDrop = [CCCallFuncN actionWithTarget:self selector:@selector(spiderDidDrop:)];
	CCSequence* sequence = [CCSequence actions:easeEnd, callDidDrop, nil];
...
-(void) runGrassMoveSequence:(CCSprite*)grass
{
	...
	// This is the sequence which controls the spiders' movement. A CCCallFuncN is used to reset the
	// spider once it has moved outside the lower border of the screen, which is when it can be re-used.
	CGPoint belowScreenPosition = CGPointMake(grass.position.x, -(3 * [grass texture].contentSize.height));
	CCMoveTo* moveEnd = [CCMoveTo actionWithDuration:spiderMoveDuration position:belowScreenPosition];
	CCEaseBackInOut* easeEnd = [CCEaseBackInOut actionWithAction:moveEnd];
	CCCallFuncN* callDidDrop = [CCCallFuncN actionWithTarget:self selector:@selector(spiderDidDrop:)];
	CCSequence* sequence = [CCSequence actions:easeEnd, callDidDrop, nil];
...
-(void) spiderDidDrop:(id)sender
{
    ...
    CCSprite* rock = (CCSprite*)sender;
	CCSprite* grass = (CCSprite*)sender;
    
	// move the spider back up outside the top of the screen
	CGPoint pos = spider.position;
    CGPoint posR = rock.position;
    CGPoint posG = rock.position;
	CGSize screenSize = [[CCDirector sharedDirector] winSize];
	pos.y = screenSize.height + [spider texture].contentSize.height;
	posR.y = screenSize.height + [rock texture].contentSize.height;
    posG.y = screenSize.height + [grass texture].contentSize.height;
    spider.position = pos;
    rock.position = posR;
    grass.position = posG;
}
...
-(void) update:(ccTime)delta
{
	...
	else
	{
        ...
        runningRate = (currentTime * runMultiplier);
        if (score < runningRate)
		{
			score = runningRate;
...
{	
    ...
    for (NSUInteger i = 0; i < numSpiders; i++)
	{
		...
		if (actualDistance < maxCollisionDistance)
		{
            ...
            [self showGameOver];
	//TBD	[[SimpleAudioEngine sharedEngine] playEffect:@"alien-sfx.caf"];
            
		}
      	
    }
    
    for (NSUInteger j = 0; j < numSpiders; j++) {
        
        CCSprite* rock = [spiders objectAtIndex:j];        
		if ([rock numberOfRunningActions] == 0)
		{
			// This spider isn't even moving so we can skip checking it.
			continue;
		...
		// on the node positions, you don't need to take into account any offsets which you would have if the
		// position would coincide with the image's lower left corner for example.
		float actualDistRock = ccpDistance(player.position, rock.position);
        
        
        
		
		// Are the two objects closer than allowed?
		if (actualDistRock < maxCollisionDistance)
		{
            [self showGameOver];
...
// detect when you have achieved certain unlockables throughout the game


-(void) displayAchievement {
    NSLog(@"displayAchievement custom method activated! Now wait for the conditional");
    CGSize screenSize = [[CCDirector sharedDirector] winSize];
...
{
  //  CCSprite* grass;
  //  CGRect rect1 = [self positionRect:grass];
  //  CGRect rect2 = [self positionRect:player];
    
  //  if(CGRectIntersectsRect(rect1,rect2)) {
  //  runMultiplier = runMultiplier * 1.5;
  //  }
}
...
-(void) setScreenSaverEnabled:(bool)enabled
    ...
    -(void) showGameOver
    {
       ...
       //     [self runSpiderWiggleSequence:spider];
        ...
        CCLabelTTF* gameOver = [CCLabelTTF labelWithString:@"FATALITY!" fontName:@"Marker Felt" fontSize:60];
        ...
        NSString* toPlayAgain = @"That move was fatal. Tap screen.";
        ...
        CCLabelTTF* touch = [CCLabelTTF labelWithString:toPlayAgain fontName:@"HelveticaNeue-Light" fontSize:20];
